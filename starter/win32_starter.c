...
static int luafunc_showconsole(void *L)
{
...
static int luafunc_hideconsole(void *L)
{
  HWND cwnd = GetConsoleWindow();
...
static const char *luacode_con =
"local msg = _ERRMSG; _ERRMSG = nil "
"local arg = _ARG or {}; _ARG = nil "
"xpcall("
"function() "
"(loadfile 'src/main.lua')(unpack(arg)) end,"
"function(err) ShowConsole(); print('Uncaught lua script exception',debug.traceback(err)); print('Press Enter...'); io.read() end)"
;
  ...
  if (hinstLib != NULL) {
      ...
      lua_createtable && lua_pushstring && lua_rawseti)
    {
      ...
      if (L!=NULL) {
        ...
        lua_pushcclosure(L,luafunc_hideconsole,0);
        lua_setfield(L,LUA_GLOBALSINDEX,"HideConsole");
        lua_pushcclosure(L,luafunc_showconsole,0);
        lua_setfield(L,LUA_GLOBALSINDEX,"ShowConsole");

        luaL_openlibs(L);

        lua_pushcclosure(L,luafunc_mbox,0);
        lua_setfield(L,LUA_GLOBALSINDEX,"_ERRMSG");
        if (luaL_loadbuffer(L,luacode_con,strlen(luacode_con),"Initializer") == 0)
          lua_pcall(L,0,0,0);
        else
          MessageBox(NULL,
