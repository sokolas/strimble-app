...
if [ $BUILD_WXWIDGETS ]; then
  ...
  #TODO download webview2 sdk https://www.nuget.org/packages/Microsoft.Web.WebView2
  ...
  make install
  #TODO copy wxWidgets/3rdparty/webview2/build/(x86|x64)?/WebView2Loader.dll
  cd ..
...
if [ $BUILD_WXLUA ]; then
  ...
  cmake -G "MSYS Makefiles" -DCMAKE_INSTALL_PREFIX="$INSTALL_DIR" -DCMAKE_BUILD_TYPE=$WXLUABUILD -DBUILD_SHARED_LIBS=FALSE \
    ...
    -DwxWidgets_COMPONENTS="webview;xrc;xml;media;richtext;gl;html;aui;adv;core;net;base" \
    -DwxLuaBind_COMPONENTS="webview;xrc;xml;media;richtext;gl;html;aui;adv;core;net;base" \
...
if [ $BUILD_LUASOCKET ]; then
  ...
  gcc $BUILD_FLAGS -I "$INSTALL_DIR/include/lua" -o "$INSTALL_DIR/lib/lua/$LUAV/mime/core.dll" src/mime.c -llua$LUAV \
    || { echo "Error: failed to build LuaSocket mime"; exit 1; }
  gcc $BUILD_FLAGS -I "$INSTALL_DIR/include/lua" -DLUASOCKET_INET_PTON -D_WIN32_WINNT=0x0501 -o "$INSTALL_DIR/lib/lua/$LUAV/socket/core.dll" \
...
if [ $BUILD_LFS ]; then
  ...
  cd "$LFS_BASENAME/src"
  mkdir -p "$INSTALL_DIR/lib/lua/$LUAV/"
  gcc $BUILD_FLAGS -I "$INSTALL_DIR/include/lua" -o "$INSTALL_DIR/lib/lua/$LUAV/lfs.dll" lfs.c -llua$LUAV \
...
if [ $BUILD_LUASEC ]; then
  ...
  RANLIB="$(which ranlib)" perl ./Configure mingw64 shared
  make depend
  make
  ...
  cd "$LUASEC_BASENAME"
  mkdir -p "$INSTALL_DIR/lib/lua/$LUAV/"
  gcc $BUILD_FLAGS -I "$INSTALL_DIR/include/lua" -o "$INSTALL_DIR/lib/lua/$LUAV/ssl.dll" \
...
if [ $BUILD_WINAPI ]; then
  ...
  cd "$WINAPI_BASENAME"
  mkdir -p "$INSTALL_DIR/lib/lua/$LUAV/"
  gcc $BUILD_FLAGS -DPSAPI_VERSION=1 -I "$INSTALL_DIR/include/lua" -o "$INSTALL_DIR/lib/lua/$LUAV/winapi.dll" winapi.c wutils.c -lpsapi -lmpr -llua$LUAV \
